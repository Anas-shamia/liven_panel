<template>
    <div class="mb-6 bg-white-900 custom-shadow rounded-lg px-8 py-10 3sm:px-4 3sm:py-5 3sm:mb-4 relative z-2">
        <div class="flex items-center mb-4 3sm:mb-2">
            ...
            <span class="text-gray-600 font-medium 3sm:font-normal text-xl 4xl:text-lg 3sm:text-base" v-if="!open">{{form.date}}</span>
            <div v-if="open">
                <v-date-picker
                        v-model='form.date'
                        :popover="popover"
        ...
        <p class="text-gray-600 font-medium 3sm:font-normal text-xl 4xl:text-lg 3sm:text-base 3sm:mb-2 mb-4">
            Subscription End: <span class="pl-4">{{form.dateEnd}}</span></p>
    ...
    export default {
        ...
        methods: {
            ...
            subscribeEnd() {
                ...
                let mm = (parseInt($endDate.substring(0, 2)) + 1);
                let dd = $endDate.substring(3, 5);
                let yyyy = $endDate.substring(6, 10);
                $endDate = '0' + mm + '-' + dd + '-' + yyyy;
                return this.form.dateEnd = $endDate;
            },
            openEdit() {
                this.open = !this.open;
            },
